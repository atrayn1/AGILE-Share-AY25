...
def LOI(data):
    ...
    #For long dwells ths will be iterative so O(n) but I think we can upper bound it there
    ...
    #df.values converts to a 2d numpy array which for us may actually be the best choice
    #At leat for proof of concept

    #So at this point the dataframe needs to be exactly formated the same so column indices are consistent
    #TODO
    data_v = data.values
    ...
    for index in range(0, len(data_v)):
        ...
        while True:
            ...
            #If the geohashes are not the same or we have reached the end of the array
            if (c_geohash != n_geohash) or (index-1 == len(data_v)):
                break; #Exit the while Loop
            
            index += 1 #Go to the next row

        #Now that we have broek nout of the loop we comare the time from start_index to index
        #If this is above a certain threshhold we mark it or throw it into a new filtered dataframe
